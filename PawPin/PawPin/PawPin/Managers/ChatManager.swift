...
class ChatManager: ObservableObject {
    ...
    struct ChatSession: Codable, Identifiable, Hashable {
        let id: UUID
        let createdAt: Date? // mapped as optional since created_at is missing from schema
        let user1Id: UUID
        let user2Id: UUID
        let reportId: UUID?
        let updatedAt: Date
    ...
    struct ChatMessageModel: Codable, Identifiable, Hashable {
        ...
        let chatId: UUID
        let senderId: UUID
        let content: String? // mapped to database column 'text'
        let imageUrl: String?
        let createdAt: Date
        let isRead: Bool
        ...
        enum CodingKeys: String, CodingKey {
            ...
            case content = "text" // DB column is 'text'
