...
struct SettingsView: View {
    ...
    @State private var showContactSupport   = false  // ← new
    ...
    var body: some View {
                ...
                Form {
                    ...
                    Section(header: Text("Account").font(.footnote).bold()) {
                        Button {
                            ...
                            SettingsRow(icon: "person.circle.fill", label: "Edit Profile")
                        }
                        Button {
                    ...
                    Section(header: Text("Preferences").font(.footnote).bold()) {
                        ...
                        Picker(selection: $appTheme,
                               label: Label("Appearance", systemImage: "paintpalette.fill")) {
                    ...
                    // ── Support ── (new section)
                    Section(header: Text("Support").font(.footnote).bold()) {
                        Button {
                            showContactSupport = true
                        } label: {
                            SettingsRow(icon: "headset", label: "Contact Support")
                        }
                    }
                    .listRowBackground(colorScheme == .dark ? Color(.secondarySystemBackground) : Color.white)
                    ...
                    Section(header: Text("Legal & Safety").font(.footnote).bold()) {
                        Button {
                            ...
                            SettingsRow(icon: "shield.righthalf.filled", label: "Privacy Policy")
                        }
                        Button {
                            showTerms = true
                        } label: {
        ...
        .sheet(isPresented: $showContactSupport) { ContactSupportView() }   // ← new
        ...
        .alert("Delete Account Permanently", isPresented: $showDeleteConfirmation) {
            Button("Cancel", role: .cancel) {}
            Button("Delete Permanently", role: .destructive) { performDeleteAccount() }
        } message: {
    ...
    private func performLogout() {
        isLoading = true
        Task {
            do { try await AuthManager.shared.logout() }
            catch { print("Failed to log out: \(error)") }
            isLoading = false
        }
    }
    ...
    private func performDeleteAccount() {
        isLoading = true
        Task {
            do { try await AuthManager.shared.deleteAccount() }
            catch { print("Failed to delete account: \(error)") }
            isLoading = false
        }
    }
}
...
struct EditProfileView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 24) {
                ...
                PrimaryButton(title: "Save Changes", isLoading: isLoading) { saveProfile() }
                    .padding(.horizontal)
                    .padding(.top, 8)
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("Close") { dismiss() }.foregroundColor(Color.brand)
                }
            }
        }
    }
    ...
    private func saveProfile() {
        guard !fullName.isEmpty else { errorMessage = "Name cannot be empty."; return }
        guard let userId = AuthManager.shared.currentUserID else { return }
        isLoading = true; errorMessage = nil
        Task {
            do {
                try await SupabaseManager.shared.client
                    .from("users").update(["full_name": fullName])
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 20) {
                ...
                CustomTextField(icon: "lock.fill", placeholder: "New Password", text: $newPassword, isSecure: true)
                    .padding(.horizontal)
                CustomTextField(icon: "lock.fill", placeholder: "Confirm New Password", text: $confirmPassword, isSecure: true)
                    .padding(.horizontal)
                ...
                PrimaryButton(title: "Change Password", isLoading: isLoading) { updatePassword() }
...
struct LegalSheetView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView { Text(content).font(.body).padding() }
                .navigationTitle(title)
                .navigationBarTitleDisplayMode(.inline)
                .toolbar {
                    ToolbarItem(placement: .navigationBarTrailing) {
                        Button("Done") { dismiss() }.foregroundColor(Color.brand)
                    }
                }
        }
    }
