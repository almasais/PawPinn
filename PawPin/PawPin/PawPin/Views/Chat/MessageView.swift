...
struct MessageView: View {
        ...
        VStack(spacing: 0) {
            ScrollViewReader { proxy in
                ...
                .onChange(of: chatManager.currentMessages.count) { _ in
                    ...
                    if !isCurrentUser {
                        scheduleLocalNotificationFor(
                            messageId: last.id,
                            contentText: last.content,
                            imageUrl: last.imageUrl,
                            from: chatPreview.username
                        )
                    }
                }
            }
            ...
            if let image = pendingImage {
                HStack(alignment: .bottom) {
                    ZStack(alignment: .topTrailing) {
                        ...
                        Button {
                            withAnimation(.easeInOut(duration: 0.2)) { pendingImage = nil }
                        } label: {
            ...
            HStack(spacing: 12) {
                ...
                Button { sendPendingMessage() } label: {
                    if isUploading {
                        ProgressView()
            ...
            .overlay(Divider().background(Color.gray.opacity(0.15)), alignment: .top)
        }
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            ToolbarItem(placement: .principal) {
                HStack(spacing: 8) {
                    AvatarView(url: chatPreview.avatarURL, size: 34)
                    Text(chatPreview.username).font(.headline)
                }
            }
        }
        .onAppear {
            ...
            Task {
                if let data = try? await item.loadTransferable(type: Data.self),
                   let uiImage = UIImage(data: data) {
                    await MainActor.run {
                        withAnimation { self.pendingImage = uiImage }
                    }
                }
            }
        }
    ...
    private func scheduleLocalNotificationFor(messageId: UUID, contentText: String?, imageUrl: String?, from senderName: String) {
        ...
        if let text = contentText, !text.isEmpty { content.body = text }
        else if imageUrl != nil { content.body = "📷 Photo" }
    ...
    private func deleteMessage(id: UUID) async {
        do {
            try await SupabaseManager.shared.client
                .from("messages").delete().eq("id", value: id).execute()
            await MainActor.run {
